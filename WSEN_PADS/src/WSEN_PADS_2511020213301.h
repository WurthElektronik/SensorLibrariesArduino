...
typedef enum {
	PADS_ODR_PowerDown = 0,			/* single conversion/power down */
	PADS_ODR_1HZ = 1,				/* 1Hz */
	PADS_ODR_10HZ = 2,				/* 10Hz */
	PADS_ODR_25HZ = 3,				/* 25Hz */
	PADS_ODR_50HZ = 4,				/* 50Hz */
	PADS_ODR_75HZ = 5,				/* 75Hz */
	PADS_ODR_100HZ = 6,				/* 100Hz */
	PADS_ODR_200HZ = 7				/* 200Hz */
...
typedef enum {
	...
	int8_t PADS_getRAWTemperature(uint16_t *rawTemp);
	...
	/*Fifo Data Out */
	int8_t PADS_getFifoRAWTemperature(uint16_t *rawTemp);
	int8_t PADS_getFifoRAWPressure(uint32_t *rawPres);
